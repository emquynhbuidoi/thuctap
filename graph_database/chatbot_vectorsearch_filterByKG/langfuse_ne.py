...
class LangfuseHandler:
    ...
    def get_chats_memory_longterm(self, session_id):
        ...
        if len(traces) > 0:
            for trace in traces:
                ...
                items = re.split(r'\.\s+|\.$', trace_output)
    ...
    def update_current_trace(self, name, session_id, input, output):
        ...
        for item in output.strip().split('.'):        # Vì thông tin lưu vào memory nên là 1 fact <=> 1 câu... https://microsoft.github.io/autogen/stable/user-guide/agentchat-user-guide/memory.html
            if item.strip(): 
                self.chats_memory_short_term.append(
                    {
                        'title': "Doc Memory From Session",
