...
from config import INDEX_NAME_PDF, INDEX_NAME_HYBRID
...
async def get_rag_prompt(query: str, tmp_file_path: Optional[str] = None):
    ...
    if tmp_file_path is not None:
        ...
        # print("\n\nQUERY: ", query)
        # print("CONTEXT: ", context_from_pdf)
...
async def get_anew_prmopt(query: str, client: AzureAIChatCompletionClient):
    ...
    prompt_agent = AssistantAgent(
        ...
        system_message="""Bạn Chỉ là một prompt engineer giỏi, với phong cách sửa các prompt dài dòng thành các prompt ngắn gọn, liệt kê theo ý chính tập trung vào thông tin liên quan tới câu hỏi.
                        Bạn không thể trả lời câu hỏi, bạn chỉ có thể tóm tắt lại câu hỏi cho hệ thống.
                        Mục tiêu của bạn chỉ là chuyển đổi câu prompt đầu vào khó hiểu thành 1 prompt ngắn gọn theo ý và liệt kê dễ hiểu, tập trung vào thông tin trả lời câu hỏi.
