...
import uuid
session_id_main = str(uuid.uuid4())
...
async def get_rag_prompt(query: str):
    if "Thông tin cung cấp được lấy từ file:" in query:
        return query
    retrieval_context = await hybrid_search(search_query=query, top_k=3, model=model, INDEX_NAME=INDEX_NAME_HYBRID)
...
async def get_anew_prmopt(query: str, client: AzureAIChatCompletionClient):
    ...
    langfuse_context.update_current_observation(
                model="MODEL_CHATGPT_PROMPT",
                usage_details={
                    "input": response.chat_message.models_usage.prompt_tokens,
                    'output': response.chat_message.models_usage.completion_tokens
                },
                cost_details={
                    "input": response.chat_message.models_usage.prompt_tokens*0.00011,  # Ví dụ: có giá 0.0000011 USD
                    "output": response.chat_message.models_usage.completion_tokens*0.00011,  # Ví dụ: có giá 0.0000011 USD
                }
            )
    return response.chat_message.content

async def hybrid_search_tool_func(*, search_query: str):
    return await hybrid_search(search_query=search_query, top_k=3, model=model, INDEX_NAME=INDEX_NAME_HYBRID)

@observe(as_type="generation")
async def get_response(prompt: str, session_id_choiced: Optional[str] = None):
    ...
    langfuse_context.update_current_observation(
        ...
        cost_details={
            "input": prompt_tokens * 0.0001,  # Ví dụ: mỗi token có giá 0.000001 USD
            "output": completion_tokens * 0.0002,  # Ví dụ: mỗi token có giá 0.000002 USD
        }
    )

    if session_id_choiced is not None:
        session_id = session_id_choiced
    else: 
        session_id = session_id_main
    
    if "Dựa vào file trả lời câu hỏi:" in prompt:
        prompt = prompt.split("Dựa vào file trả lời câu hỏi:")[1]
