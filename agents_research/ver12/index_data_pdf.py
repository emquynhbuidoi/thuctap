...
def index_data(documents: list[dict], INDEX_NAME: str):
    """
        input:
    ...
    _ = _insert_documents(es=es, documents=documents, INDEX_NAME=INDEX_NAME)
...
def _insert_documents(es: Elasticsearch, documents: List[dict], INDEX_NAME: str):
    """
        input:
            es: Elasticsearch | đối tượng Elasticsearch đã được kết nối với elasticsearch local.
            documents: list[dict] | chứa thông tin các nodes vừa mới parser từ pdf sang doc và chunking.
